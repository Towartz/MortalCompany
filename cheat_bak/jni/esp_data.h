...
struct TouchData {
    float x, y;         // screen position (game coords — Unity Input space)
    bool  isDown;       // touch began this frame
    bool  isUp;         // touch ended this frame
    bool  isTouching;   // continuous: touch is currently active (true every frame while held)
    bool  fresh;        // set by main thread, cleared by render thread after consume
};
...
struct ESPSharedData {
    ...
    int        itemCount;
    TouchData  touch;
    bool       dataReady;     // main thread sets true after populate
    bool       drawPlayers;
    bool       drawObjects;
    bool       drawBoxes;
    bool       drawTracelines;
    bool       drawLabels;
    float      maxDist;
    float      gameW, gameH;  // game render resolution (cached for scaling)
};
