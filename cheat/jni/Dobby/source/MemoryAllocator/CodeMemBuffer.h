...
struct MemBuffer {
  ...
  MemBlock dup() {
    uint8_t *copy_buf = (uint8_t *)malloc(buffer_size);
    if (copy_buf == nullptr)
      return MemBlock(0, 0);
    std::memcpy(copy_buf, buffer, buffer_size);
  ...
  bool read(uint32_t offset, void *out_buffer, int size) {
    if (offset + size > this->buffer_size)
      return false;
    std::memcpy(out_buffer, this->buffer + offset, size);
    return true;
  }

  bool write(uint32_t offset, void *in_buffer, int size) {
    if (offset + size > this->buffer_size)
      return false;
    std::memcpy(this->buffer + offset, in_buffer, size);
    return true;
  }
