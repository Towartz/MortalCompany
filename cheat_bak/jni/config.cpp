...
#define CONFIG_LOG_TAG "FBK"
#define LOGD(...) __android_log_print(ANDROID_LOG_DEBUG, CONFIG_LOG_TAG, __VA_ARGS__)

Config g_config;
const char* g_configPath = "/sdcard/cheat.cfg";

// Auto-reload: track file modification time
static time_t g_lastConfigMtime = 0;

static time_t GetConfigMtime() {
    struct stat st;
    if (stat(g_configPath, &st) != 0) return 0;
    return st.st_mtime;
}

// Call periodically (e.g. every 60 frames) to reload if file changed
void CheckConfigReload() {
    time_t mtime = GetConfigMtime();
    if (mtime != 0 && mtime != g_lastConfigMtime) {
        g_lastConfigMtime = mtime;
        LoadConfig();
        LOGD("config auto-reloaded");
    }
}
...
void LoadConfig() {
    g_lastConfigMtime = GetConfigMtime(); // Track initial mtime
